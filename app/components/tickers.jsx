...
const Tickers = async () => {
    ...
    const TickerItem = ({ item }) => {
        ...
        return (
            <div className="flex items-center space-x-3 px-6 py-2 bg-white/10 backdrop-blur-sm rounded-full border border-white/20 whitespace-nowrap">
                {/* Company Logo */}
                <div className="relative w-6 h-6 rounded-full overflow-hidden bg-white bg-opacity-20 flex-shrink-0">
                    <Image 
                ...
                <div className={`flex items-center space-x-1 px-2 py-1 rounded-full text-xs font-medium ${
                    ...
                    <span>{change.isPositive ? '+' : ''}{Math.abs(change.percent).toFixed(2)}%</span>
                </div>
            </div>
        );
    };
    ...
    const scrollingData = [...refineData, ...refineData, ...refineData];

    return (
        <div className="w-full bg-gradient-to-r from-blue-900 via-blue-800 to-indigo-900 py-3 border-y border-blue-700 overflow-hidden relative">
            {/* Background Pattern */}
            ...
            {/* Live Indicator */}
            <div className="absolute left-4 top-1/2 -translate-y-1/2 flex items-center space-x-2 z-10 bg-blue-500">
                <div className="w-2 h-2 bg-red-500 rounded-full animate-pulse"></div>
                <span className="text-xs font-medium text-white opacity-80 uppercase tracking-wide">Live</span>
            </div>

            {/* Error Indicator */}
            {hasError && (
                <div className="absolute right-4 top-1/2 -translate-y-1/2 z-10">
                    <div className="bg-yellow-500 bg-opacity-20 text-yellow-300 px-2 py-1 rounded-full text-xs font-medium border border-yellow-400 border-opacity-30">
                        Cached
                    </div>
                </div>
            )}
            ...
            {/* Scrolling Content */}
            <div className="flex animate-pulse hover:animate-none ml-20 mr-20" 
                 style={{
                     animation: 'scroll-horizontal 45s linear infinite'
                 }}>
                <div className="flex space-x-4 whitespace-nowrap">
                    {scrollingData.map((item, index) => (
                        <TickerItem key={`ticker-${index}`} item={item} />
                    ))}
                </div>
            </div>
        </div>
