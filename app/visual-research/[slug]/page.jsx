...
export default function DSEMarketAnalysis() {
    ...
    const [historicalWithDMA, setHistoricalWithDMA] = useState([]);
    const [advanceDeclineRatio, setAdvanceDeclineRatio] = useState(0);
    const [maxSectorPercent, setMaxSectorPercent] = useState([]);
    const [sectorPerformance, setSectorPerformance] = useState([]);
    const [marketSummary, setMarketSummary] = useState({});
    const [categoryBreadth, setCategoryBreadth] = useState([]);
    const [circuitBreakers, setCircuitBreakers] = useState({ upper: [], lower: [] });
    const [marketBreadth, setMarketBreadth] = useState({ gainers: {}, losers: {}, unchanged: {}, commentary: {} });
    const [indices, setIndices] = useState({ dsex: {}, dses: {}, ds30: {} });
    const [technicalLevels, setTechnicalLevels] = useState({});
    const [topValue, setTopValue] = useState([]);
    const [topGainers, setTopGainers] = useState([]);
    const [topLosers, setTopLosers] = useState([]);
    const [ytdPerformance, setYtdPerformance] = useState({});
    const [volumeSpikes, setVolumeSpikes] = useState({ totalCount: 0, top5: [] });
    const [sectorCommentary, setSectorCommentary] = useState({ en: '', bn: '' });
    const [intradayData, setIntradayData] = useState([]);
    const [blockMarket, setBlockMarket] = useState({ summary: {}, top5: [] });
    const [commentary, setCommentary] = useState({ en: '', bn: '' });
    const [date, setDate] = useState('');
    // State for main commentary language
    const [mainCommentaryEnglish, setMainCommentaryEnglish] = useState(true);
    useEffect(() => {
        const isMarketPositive = dailyData?.indices?.dsex?.change >= 0;
        setAccentColor(isMarketPositive ? '#16a34a' : '#dc2626');
        setIsPositive(isMarketPositive);
        setHistoricalWithDMA(dailyData?.historicalData?.map(d => ({
            ...d,
            dma20: technicalLevels.dma20
        })));
        setAdvanceDeclineRatio((dailyData?.marketSummary?.issuesAdvanced / dailyData?.marketSummary?.issuesDeclined).toFixed(2));
        setMaxSectorPercent(Math.max(dailyData?.sectorPerformance?.length > 0 && dailyData.sectorPerformance.map(s => s.percent)));
        setSectorPerformance(dailyData?.sectorPerformance || []);
        setMarketSummary(dailyData?.marketSummary || {});
        setCategoryBreadth(dailyData?.categoryBreadth || []);
        setCircuitBreakers(dailyData?.circuitBreakers || { upper: [], lower: [] });
        setMarketBreadth(dailyData?.marketBreadth || { gainers: {}, losers: {}, unchanged: {}, commentary: {} });
        setIndices(dailyData?.indices || { dsex: {}, dses: {}, ds30: {} });
        setTechnicalLevels(dailyData?.technicalLevels || {});
        setTopValue(dailyData?.topValue || []);
        setTopGainers(dailyData?.topGainers || []);
        setTopLosers(dailyData?.topLosers || []);
        setYtdPerformance(dailyData?.ytdPerformance || {});
        setVolumeSpikes(dailyData?.volumeSpikes || { totalCount: 0, top5: [] });
        setSectorCommentary(dailyData?.sectorCommentary || { en: '', bn: '' });
        setIntradayData(dailyData?.intradayData || []);
        setBlockMarket(dailyData?.blockMarket || { summary: {}, top5: [] });
        setCommentary(dailyData?.commentary || { en: '', bn: '' });
    }, [dailyData]);
    ...
    useEffect(() => {
        fetch(`/api/visual-research/DSE-Market-Data-Dec-10-2025.json`)
            ...
            .catch(error => {
                console.error("Error fetching DSE Market Data:", error);
            });
    }, []);
    return (
        <div className='font-poppins' style={{
            ...
            <div style={{
                ...
                {/* Market Summary */}
                <div style={{ background: '#f8fafc', borderRadius: '12px', padding: '16px', marginBottom: '20px', border: '1px solid #e2e8f0', overflowX: 'auto' }}>
                ...
                {/* Circuit Breakers */}
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '12px', marginBottom: '20px', overflowX: 'auto' }}>
                ...
                {/* Volume Spike Alert */}
                <div style={{ background: '#fefce8', borderRadius: '12px', padding: '16px', marginBottom: '20px', border: '1px solid #fef08a', overflowX: 'auto' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '12px' }}>
